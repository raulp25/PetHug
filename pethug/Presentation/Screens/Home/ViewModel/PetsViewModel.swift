...
final class PetsViewModel {
    ...
    init(fetchPetsUC: DefaultFetchPetsUC, filterPetsUC: DefaultFilterPetsUC, likedPetUC: DefaultLikePetUC) {
...
//    }
    ...
    func addLikeUid(pet: Pet) throws -> Pet{
        ...
        if pet.likedByUsers.contains(uid) {
            throw PetsError.defaultCustom("Ya le has dado like al animal")
        }
        ...
        if let index = pets.firstIndex(where: { pet in
            ...
            print("pet.likedbyusers 919: => \(updatePet)")
            pets[index] = updatePet
            return updatePet
        }
        
        
        throw PetsError.defaultCustom("No se encontro el indice del animal")
    }
